...
const Project = () => {
 ...
 const debugFileTree = () => {
    ...
    Object.keys(fileTree).forEach(key => {
      ...
      console.log('Content preview:', fileTree[key]?.file?.contents?.substring(0, 100) + '...')
    })
  }

  // File validation function
  const validateFileTree = (fileTree) => {
    const forbiddenFiles = ['routes/index.js', 'controllers/index.js', 'models/index.js'];
    const validatedTree = { ...fileTree };
    
    // Remove forbidden files
    forbiddenFiles.forEach(forbidden => {
      if (validatedTree[forbidden]) {
        console.warn(`Removing forbidden file: ${forbidden}`);
        delete validatedTree[forbidden];
      }
    });
    
    // Validate package.json
    if (validatedTree['package.json']?.file?.contents) {
      try {
        const packageData = JSON.parse(validatedTree['package.json'].file.contents);
        ...
        // Add default scripts if missing
        if (!packageData.scripts.start && !packageData.scripts.dev) {
          packageData.scripts.start = "node app.js";
        }
        
        validatedTree['package.json'].file.contents = JSON.stringify(packageData, null, 2);
      } catch (e) {
        console.warn('Invalid package.json, creating default');
        validatedTree['package.json'] = {
          file: {
  ...
  const runProject = async () => {
    setIsRunning(true)
    try {
      // Transform the file tree to handle nested paths properly
      const transformedFileTree = {}
      
      Object.keys(fileTree).forEach(filePath => {
        const pathParts = filePath.split('/')
        let currentLevel = transformedFileTree
        ...
        currentLevel[fileName] = fileTree[filePath]
      })
      
      console.log('Transformed file tree:', transformedFileTree)
      
      // Check if package.json exists, if not create a basic one
      if (!transformedFileTree['package.json']) {
        transformedFileTree['package.json'] = {
          file: {
            contents: JSON.stringify({
      ...
      // Mount the properly structured file tree
      await webContainer.mount(transformedFileTree)
      
      // Check if package.json has dependencies before installing
      const packageJsonContent = transformedFileTree['package.json']?.file?.contents
      ...
      if (packageJsonContent) {
        try {
          ...
          hasDepedencies = packageData.dependencies && Object.keys(packageData.dependencies).length > 0
        } catch (e) {
      ...
      if (hasDepedencies) {
        ...
        try {
          ...
          if (installExitCode === 0) {
            ...
            console.warn('npm install failed, cannot run project with dependencies')
            setIsRunning(false)
            alert('⚠️ Dependencies installation failed!\n\nThis usually happens when:\n• Package.json has invalid dependencies\n• Network connectivity issues\n• WebContainer resource limits\n\nPlease ask AI to create a simpler project without complex dependencies.')
            return
          }
        } catch (installError) {
          console.warn('Install failed:', installError)
          setIsRunning(false)
          alert('❌ Installation Error!\n\nCannot install project dependencies. Try asking AI to create a basic Node.js project without external packages.')
          return
        }
      } else {
        installSuccess = true // No dependencies needed
      }
      
      // Only proceed if install was successful or no dependencies needed
      if (!installSuccess) {
        setIsRunning(false)
        return
      }
      ...
      if (packageJsonContent) {
        try {
          ...
          console.warn('Could not parse package.json for start command:', e)
        }
      }
      
      // Start the project
      console.log(`Starting project with command: npm ${startCommand}`)
      const tempRunProcess = await webContainer.spawn("npm", [startCommand])
      ...
      // Also listen for process exit in case of errors
      ...
      setTimeout(() => {
        if (isRunning) {
          console.warn('Project startup is taking longer than expected...')
        }
      }, 30000)
      ...
      alert(`Error running project: ${errorMessage}\n\nPlease check the console for more details.`)
    }
  }
  return (
